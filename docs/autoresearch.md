...
- When a live applicator is used, an `AuditLog` can be forwarded (see
  [safety.md](safety.md)) so every apply/rollback lands on the durable trail.
...
- With no live engine attached the loop uses a `DryRunApplicator`, so autoresearch
  ...
  catalog claims, never counted as a proven gain. No audit trail is written for a
  dry-run (auditing no-op applies would be dishonest).
...
run = autoresearch(trace, applicator=applicator, policy=policy, audit=audit)
