...
| `mxfp4` | Triton (triton_kernels), bf16 MACs [estimated: triton_kernels not read] | TRT-LLM **MXFP4 x bf16**, bf16 MACs, per-rank dims padded to 256 | AITER CK 2-stage, scaled f8f6f4 MFMA (fp4 MACs), MXFP4 activations by a separate kernel fused with the MoE sort, per-rank dims padded to 256 |
...
| `fp8_block128` | fp8 MACs, activations `fp8_group128` | same | AITER `fmoe_fp8_blockscale_g1u1` (or CK 2-stage at M <= 32), fp8 e4m3fn MACs, activations `fp8_group128` by a separate kernel, `shuffle_weight` is layout-only |
...
Everything above is validated against source: checkpoint headers and configs, vLLM v0.19.1, AITER v0.1.10.post2 and its CK submodule. None of it is validated against hardware. The efficiency band (0.55 to 0.95), the 2 us launch floor, and the one rule still marked estimated (MXFP4 via Triton on Hopper, because `triton_kernels` was not read) are the parts a measurement should replace first. MI355X peaks are not stated anywhere in the AITER or vLLM trees; the catalogue's figures come from AMD's published dense rates. H-001 and H-002 are the first two measurements registered for that.
