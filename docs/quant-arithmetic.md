...
Every engine fact is read from vLLM v0.19.1 (commit `b1388b1f`), the version the K2.6 case pinned. Engine paths are relative to `vllm/model_executor/layers/` unless they start with `vllm/`.
...
|---|---|---|---|
| `nvfp4` | Marlin W4A16, **bf16** MACs, streams 0.5625, no activation quant | TRT-LLM W4A4, fp4 MACs, one `scaled_fp4_quant` per MoE input | no rule pinned (ladder fallback, flagged) |
| `mxfp4` | Triton (triton_kernels), bf16 MACs [estimated] | TRT-LLM **MXFP4 x bf16**, bf16 MACs, hidden and intermediate padded to 256 | CK / AITER, fp4 MACs [estimated] |
| `mxfp4`, forced Marlin | bf16 MACs, hidden padded to 256, intermediate to 128 | same | not available |
| `mxfp8` | **no kernel**: raises `UnsupportedExecution` | TRT-LLM, fp8 MACs, activations MXFP8 | not pinned |
| `fp8_block128` | fp8 MACs, activations `fp8_group128` | same | AITER, fp8 [estimated] |
| `int4_g32` | Marlin W4A16, bf16 MACs | Marlin W4A16, bf16 MACs | W4A16, bf16 MACs [estimated] |

Evidence for each cell is in `_EXECUTION_RULES` and `_FORCED`, with file and line. Three results were not obvious before reading the source:
...
3. **Padding is a storage-versus-execution cost the format does not show.** Marlin rounds hidden to 256 and intermediate to 128 (`oracle/mxfp4.py:380-385`), and TRT-LLM rounds both to 256 (`:386-388`). A gpt-oss-shaped 2,880 x 2,880 expert streams 9.0% more under Marlin and 13.8% more under TRT-LLM. Kimi (7,168 x 2,048) and GLM-5.2 (6,144 x 2,048) are multiples of 256, so their factor is 1.
...
Everything above is validated against source: checkpoint headers and configs, and vLLM v0.19.1 code. None of it is validated against hardware. The efficiency band (0.55 to 0.95), the 2 us launch floor, and every rule marked estimated (all CDNA4 rules, MXFP4 via Triton on Hopper) are the parts a measurement should replace first. H-001 and H-002 are the first two measurements registered for that.
