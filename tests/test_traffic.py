...
FIXTURES = Path(
    os.environ.get(
        ...
        Path(__file__).resolve().parents[1] / "benchmarks" / "traffic_replay" / "fixtures",
    )
)
...
def test_burstgpt_fixture() -> None:
...
def test_burstgpt3_layout() -> None:
...
def test_burstgpt3_defects() -> None:
...
def test_burstgpt_layouts_are_read_by_name() -> None:
...
def test_session_trace_replay_understates_reuse() -> None:
...
def test_mooncake_fixture() -> None:
...
def test_every_drop_reason_fires() -> None:
...
def test_filtering_is_not_a_defect() -> None:
...
def test_provenance_must_reconcile() -> None:
...
def test_replay_refuses_to_truncate() -> None:
...
def test_regime_axes_separate_the_traces() -> None:
...
def test_parameterized_envelope() -> None:
...
def test_bench_serve_argv() -> None:
...
def test_gui_refuses_paths_it_was_not_configured_for() -> None:
...
def test_version_guard_fires_before_launching() -> None:
...
def test_runner_builds_the_pinned_argv_and_keeps_provenance() -> None:
...
def test_join_drops_the_two_fields_that_are_wrong() -> None:
...
def test_join_attaches_the_identity_the_result_lacks() -> None:
...
def test_join_reconciles_the_real_run() -> None:
...
def test_join_catches_the_32x_truncation_after_the_fact() -> None:
...
def test_join_reads_both_directions_of_pacing_failure() -> None:
...
def test_join_accounts_for_every_result_key() -> None:
