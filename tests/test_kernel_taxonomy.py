...
def test_nonpositive_capture_window_does_not_fabricate_active_share():
    bd = summarize_kernels(
        [make_kernel("fused_moe_kernel", start_ns=0, end_ns=10)], window_ns=0
    )

    assert bd.gpu_active_share is None
    assert any("capture window duration is non-positive" in warning for warning in bd.warnings())
