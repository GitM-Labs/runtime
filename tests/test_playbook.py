...
EXAMPLES = Path(
    os.environ.get(
        ...
        Path(__file__).resolve().parents[1] / "benchmarks" / "playbook" / "examples.json",
    )
)
...
def _row(row_id: str, regime: Regime, *, tput: float = 10.0, verified: datetime | None = T0,
         **kw) -> PlaybookRow:
    """A *measured* row — the tests need selectable rows, which the shipped
...
def test_log2_ratio_is_scale_free() -> None:
...
def test_dispersion_distance_handles_flat_traces() -> None:
...
def test_linf_is_the_worst_axis() -> None:
...
def test_rate_is_not_in_the_default_axes() -> None:
...
def test_tolerance_requires_calibration() -> None:
...
def test_exact_gates_reject_before_distance() -> None:
...
def test_uncalibrated_policy_routes_to_discovery() -> None:
...
def test_exact_regime_matches_without_any_calibration() -> None:
...
def test_calibrated_policy_matches_inside_and_rejects_outside() -> None:
...
def test_precedence_exact_then_recent_then_conservative() -> None:
...
def test_synthesized_prefixes_make_a_prefix_cache_delta_a_floor() -> None:
...
def test_extrapolated_rows_lose_ties_but_not_distance() -> None:
...
def test_replay_conditions_are_carried_into_provenance() -> None:
...
def test_invalidated_row_is_kept_and_never_selected() -> None:
...
def test_a_row_cannot_exist_without_its_evidence() -> None:
...
def test_examples_ship_nothing_selectable() -> None:
...
def test_regime_is_imported_not_redeclared() -> None:
...
def _runs(**bump):
    ...
    from tests.test_traffic import _real_run
...
def test_two_bench_runs_become_a_row() -> None:
...
def test_a_row_refuses_arms_that_are_not_one_experiment() -> None:
