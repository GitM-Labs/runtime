...
def run_loop(cfg: LoopConfig) -> dict[str, Any]:
    ...
    # unverified — a candidate source, never a bypass. A live run (engine
    # applicator + audit) would thread both here; the loop's dry-run path does
    # not, so no no-op entries land on the safety trail.
    if time.time_ns() - started_ns < int(budget_s * 1e9):
